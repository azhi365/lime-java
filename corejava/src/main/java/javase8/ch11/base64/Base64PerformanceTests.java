...
public class Base64PerformanceTests {
    ...
    private static final Base64Codec[] m_codecs = {new GuavaImpl(), new JavaXmlImpl(), new Java8Impl(), new SunImpl(), new ApacheImpl(), new MiGBase64Impl(), new IHarderImpl()};
    private static final Base64ByteCodec[] m_byteCodecs = {new ApacheImpl(), new Java8Impl(), new MiGBase64Impl(), new IHarderImpl()};
...
//        System.exit(0);
        ...
        for (final Base64Codec codec : m_codecs) {
            for (int i = 1; i <= 17; ++i) {
                ...
                try {
                    ...
                    data = null;
                    System.out.println("Codec " + codec.getClass()
                            .getName() + " Failed encoding at size = " + i / 10.0 + " Gb due to " + ex);
                    ex.printStackTrace();
                    break;
                }
            }
            for (int i = 1; i <= 17; ++i) {
                ...
                try {
                    ...
                    encoded = null;
                    System.out.println("Codec " + codec.getClass()
                            .getName() + " Failed decoding at size = " + i / 30.0 * 4 + " Gb due to " + ex);
                    ex.printStackTrace();
                    break;
                }
            }
    ...
    private static TestResult testByteCodec(final Base64ByteCodec codec,
                                            final List<byte[]> buffers) throws IOException {
    ...
    private interface Base64Codec {
        String encode(final byte[] data);

        byte[] decode(final String base64) throws IOException;
    }
